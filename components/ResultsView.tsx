...
import { Flame, PartyPopper, Lightbulb, AlertCircle, CheckCircle2, ArrowRight, Dumbbell, TrendingUp, CreditCard, ShieldCheck, Coins, Wallet } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8 animate-fade-in pb-12">
      ...
      <div className="text-center space-y-2">
        <span className="inline-block px-4 py-1 rounded-full bg-slate-800 border border-slate-700 text-slate-400 text-sm font-medium uppercase tracking-wider">
        ...
        <p className="text-xl text-slate-300 max-w-2xl mx-auto font-light italic">
          "{data.summaryText}"
        </p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6">
        ...
        {/* Vibe Score Card */}
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-2xl p-6 flex flex-col md:flex-row items-center gap-8 relative overflow-hidden group hover:border-freo-500/50 transition-colors">
            ...
            <div className="relative w-48 h-48 flex-shrink-0 flex items-center justify-center">
                ...
                <div className="absolute inset-0 flex flex-col items-center justify-center">
                    ...
                    <span className="text-xs font-bold uppercase tracking-widest text-slate-400">Vibe Score</span>
                </div>
            </div>

            <div className="flex flex-col items-center md:items-start text-center md:text-left space-y-3">
                 <div className="space-y-1">
                    <span className="text-slate-400 text-sm uppercase tracking-wide">Verdict</span>
                 ...
                 <div className="flex items-center gap-2 bg-slate-900/50 px-4 py-2 rounded-lg border border-slate-700">
                    ...
                    <span className={`font-medium ${data.vibeScore > 70 ? 'text-green-400' : 'text-red-400'}`}>
                        {data.vibeScore > 70 ? 'You cooked well' : 'Cooked beyond repair?'}
                    </span>
                </div>
            </div>
        </div>
        ...
        <div className="bg-slate-800/50 backdrop-blur border border-slate-700 rounded-2xl p-6 flex flex-col hover:border-freo-500/50 transition-colors">
             ...
             <div className="flex flex-col md:flex-row items-center gap-8">
                ...
                <div className="w-full md:w-1/2 h-[250px]">
                    <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                            ...
                            <Tooltip 
                                contentStyle={{ backgroundColor: '#0f172a', borderColor: '#334155', borderRadius: '12px', color: '#f8fafc', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.1)' }}
                ...
                <div className="w-full md:w-1/2 space-y-5">
                    <div className="flex justify-between items-end border-b border-slate-700 pb-2">
                        <span className="text-slate-400 text-sm font-medium uppercase tracking-wider">Total Spend</span>
      ...
      {data.swapSuggestions && data.swapSuggestions.length > 0 && (
          <div className="bg-gradient-to-r from-slate-900 to-slate-900 border border-freo-500/30 rounded-2xl p-6 relative overflow-hidden shadow-2xl shadow-black/50">
              <div className="absolute top-0 right-0 p-4 opacity-5 pointer-events-none">
                  <TrendingUp className="w-64 h-64 text-freo-400" />
              </div>
              
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 relative z-10 border-b border-slate-800 pb-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-slate-400 text-sm">Real-time swaps to fix your life components.</p>
                    </div>
                  </div>
                  <div className="text-right hidden md:block">
                      <span className="text-xs font-bold text-freo-500 uppercase tracking-widest">Strategy</span>
                      <div className="text-slate-300 font-medium">Invest in "Future You" 🚀</div>
                  </div>
              </div>

              <div className="grid grid-cols-1 gap-4 relative z-10">
                  {data.swapSuggestions.map((swap, idx) => (
                      <div key={idx} className="bg-slate-950 rounded-xl p-5 flex flex-col md:flex-row items-center gap-6 border border-slate-800 hover:border-accent-teal/50 transition-all group">
                          
                          {/* Habit to Break */}
                          <div className="flex-1 text-center md:text-left w-full md:w-auto bg-red-500/5 p-3 rounded-lg border border-red-500/10">
                              <div className="text-[10px] text-red-400 font-black uppercase tracking-widest mb-1">STOP THE ROT 🛑</div>
                              <div className="text-slate-300 font-medium text-lg group-hover:text-white transition-colors">{swap.habitToBreak}</div>
                          </div>
                          
                          {/* Arrow */}
                          <div className="flex items-center justify-center">
                             <div className="bg-slate-800 p-2 rounded-full">
                                <ArrowRight className="w-5 h-5 text-slate-500 group-hover:text-accent-teal transition-colors" />
                             </div>
                          </div>

                          {/* Better Alternative */}
                          <div className="flex-1 text-center md:text-left w-full md:w-auto bg-green-500/5 p-3 rounded-lg border border-green-500/10">
                          ...
                          {/* Impact */}
                          <div className="md:border-l border-slate-800 md:pl-6 flex-1 text-center md:text-right w-full md:w-auto">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className={`rounded-2xl p-6 border relative overflow-hidden ${
              ...
              <p className="text-slate-200 text-lg leading-relaxed italic opacity-90">
          ...
          <div className="rounded-2xl p-6 border border-freo-500/30 bg-freo-900/10 relative overflow-hidden flex flex-col">
               <div className="absolute top-0 right-0 -mt-4 -mr-4 w-24 h-24 bg-freo-500/20 rounded-full blur-2xl" />
              ...
              <div className="flex-grow relative z-10">
                   {data.recommendedProduct && (
                       <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-freo-500/20 border border-freo-500/40 text-freo-300 text-sm font-bold mb-3">
              ...
              <div className="mt-4 pt-4 border-t border-freo-500/20 relative z-10">
                  <div className="flex items-center justify-between group cursor-pointer">
                    ...
                    <ArrowRight className="w-4 h-4 text-slate-500 group-hover:text-freo-400 transition-colors transform group-hover:translate-x-1" />
                  </div>
              </div>
          </div>
      </div>
